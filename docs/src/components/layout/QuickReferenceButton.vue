<script setup lang="ts">
import { ref, watch } from 'vue';

import QuickReferenceModal from './QuickReferenceModal.vue';

const isOpen=ref<boolean>(false);

watch(isOpen,(v)=>{
  if(v){
    document.body.style.overflow='hidden';
    return;
  }

  document.body.style.overflow='auto';
});
</script>

<template>
  <button class="quickReferenceBtn" @click="isOpen=!isOpen">
    <svg class="quickReferenceBtn-icon" xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 -960 960 960" fill="var(--sl-color-text)">
      <path d="M200-800v241-1 400-640 200-200Zm80 400h140q9-23 22-43t30-37H280v80Zm0 160h127q-5-20-6.5-40t.5-40H280v80ZM200-80q-33 0-56.5-23.5T120-160v-640q0-33 23.5-56.5T200-880h320l240 240v100q-19-8-39-12.5t-41-6.5v-41H480v-200H200v640h241q16 24 36 44.5T521-80H200Zm531-149q29-29 29-71t-29-71q-29-29-71-29t-71 29q-29 29-29 71t29 71q29 29 71 29t71-29ZM864-40 756-148q-21 14-45.5 21t-50.5 7q-75 0-127.5-52.5T480-300q0-75 52.5-127.5T660-480q75 0 127.5 52.5T840-300q0 26-7 50.5T812-204L920-96l-56 56Z"/>
    </svg>
  </button>
  <Teleport to="body">
    <QuickReferenceModal v-if="isOpen" @close="isOpen=false"/>
  </Teleport>
</template>

<style scoped>
.quickReferenceBtn{
  background-color:transparent;
  border:none;
  cursor:pointer;
}

.quickReferenceBtn-icon{
  display:block;
}
</style>
